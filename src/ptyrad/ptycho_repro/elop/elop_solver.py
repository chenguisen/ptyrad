"""ELOPOrbitalSolver — Solver for eLOP extended orbital ptychography."""
...
from ptyrad.ptycho_repro.elop.elop_model import ELOPOrbitalModel
...
class ELOPOrbitalSolver:
    def __init__(self, params, device=None, seed=None):
        ...
        self.init_variables = {
            ...
            'n_aberration_modes': D.get('n_aberration_modes', 6),
            'scan_grid_shape': tuple(D.get('scan_grid_shape', [8, 8])),
            'aperture_radius_fraction': D.get('aperture_radius_fraction', 0.4),
    ...
    def reconstruct(self):
        ...
        model = ELOPOrbitalModel(self.init_variables, self.params.get('model_params', {}), device=self.device)
        ...
        op = rp.get('output_dir', 'output/elop')
        out = safe_filename(os.path.join(op, f"ELOP_{get_time('datetime')}")) if SAVE else None
